...
%       The constraint is: dl <= surf_k - surf_k+1 <= du. When using
%       in-region costs, the surfaces are non-intersecting and ordered
%       which requires dl>0 and du>dl. If a size (1,2) array is given, the
%       same overlap parameteres are used for all surfaces. If not given
%       defaults to "no constraint" (dl=-Z, du=Z) when using only
%       on-surface costs and to "no overlap" (dl=1, du=Z) otherwise.
...
%%%%%%%%%%%%%%%%%%% HACK 20141216, not working
% w_in = -1*ones([dimension,K]);
% w_in(:,:,2:end,:) = double(r_cost(:,:,1:end-1,:))-double(r_cost(:,:,2:end,:));
%%%%%%%%%%%%%%%%%
